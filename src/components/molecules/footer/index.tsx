...
function Footer() {
  ...
  return (
    <footer >
        <Newsletter />
      <div className="container mx-auto text-xl leading-[35px] pb-4 text-[#333333] flex flex-col lg:flex-row justify-between">
        <div className="hidden lg:flex font-clash font-semibold leading-[59px] text-grey-dark text-2xl">
          <ArtsyLogo />
        </div>
        <ul className="lg:block hidden" role="nav items list">
          {navLinks.map((navLink) => (
            <Link
              ...
              key={navLink.name}
            >
              <p className="mb-4 ">{navLink.name}</p>
            </Link>
          ))}
        </ul>
        <ul className="lg:block hidden" role="nav items list">
          {infoData.map((navLink) => (
            <Link href={navLink.urlPath} key={navLink.name}>
              <p className="mb-4">{navLink.name}</p>
            </Link>
          ))}
        </ul>
        <ul className="text-sm lg:text-xl">
          <li className="lg:hidden mb-[14px]">REACH US</li>
          <li className="flex items-center mb-[14px] lg:mb-4">
            <svg
              className="h-6 w-6 lg:h-[61px] lg:w-[54px]"
              width="61"
              height="54"
              viewBox="0 0 61 54"
              fill="none"
      ...
      <p className="hidden lg:block text-center -spacing-[0.02em] text-[#333333]/[0.5] text-sm font-medium font-rubik mt-10 mb-11">
